...
class Cloud(object):
    ...
    def get_uplaod_link(self):
        ...
        upload_link_id=re.search(r"cloud\.tsinghua\.edu\.cn/ajax/u/d/(.+?)/", self.repo_upload_link).group(1)
        upload_pre="https://cloud.tsinghua.edu.cn/ajax/u/d/{0}/upload/".format(upload_link_id)
        r=self.session.get(upload_link_id, params=param, headers=headers)
        # print(r.text)
        # print(r.status_code)
    ...
    def upload(self, file_path=None, file_like=None, rel_path=""):
        file_upload_link=self.get_uplaod_link()
